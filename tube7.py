...
def my_hook(d):
    if d['status'] == 'downloading':
        # Attempt to get progress directly from the 'percent' field
        percent = d.get('percent', None)

        if percent is not None:
            pbar['value'] = int(percent)
        else:
            # Extract numeric portion from the percentage string
            percent_str = ''.join(c for c in d['_percent_str'] if c.isdigit() or c == '.')

            try:
                # Attempt to convert the extracted numeric portion to float
                pbar['value'] = int(float(percent_str))
            except (ValueError, TypeError):
                # Handle the error, for example, by setting the progress bar to an arbitrary value
                pbar['value'] = 0
